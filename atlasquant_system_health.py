...
def system_health(components:Mapping[str,Any]|None,*,now:datetime|None=None)->dict[str,Any]:
    ...
    for name in required:
        ...
        if state=="UNKNOWN":
            state="HALTED" if name in CRITICAL else "PROTECTED"
        ...
        if sev>=2 and f"{name.upper()}_UNHEALTHY" not in reasons:
    ...
    return {"state":state,"new_entries_allowed":state=="NORMAL","paper_new_entries_allowed":state=="NORMAL",
            ...
            "reasons":list(dict.fromkeys(reasons)),"components":detail,"checked_at":_utc(now).isoformat(),
