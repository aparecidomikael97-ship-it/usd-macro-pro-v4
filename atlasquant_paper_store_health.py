...
def paper_store_health(snapshot_rows:Sequence[Mapping[str,Any]]|None,ledger:Sequence[Mapping[str,Any]]|None,
    ...
    led=reconcile_active_paper(snapshot_rows,ledger); ev=audit_paper_results(results); st=audit_paper_state(snapshot_rows)
    integrity=bool(led["ok"] and ev["ok"] and st["ok"])
    reasons=[*list(led.get("reasons",[]) or []),*list(ev.get("reasons",[]) or []),*list(st.get("reasons",[]) or [])]
    return {"state":"NORMAL" if integrity else "PROTECTED","healthy":True,"available":True,"integrity_ok":integrity,
            ...
            "ledger":led,"result_evidence":ev,"state_evidence":st,"real_orders_enabled":False}
