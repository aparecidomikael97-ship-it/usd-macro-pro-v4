...
def _pair(value:Any)->str|None:
    try:
        s=normalize_fx_symbol(str(value))
        return f"{s[:3]}/{s[3:]}"
    except Exception:
        return None

def _geometry_ok(req:RiskRequest)->bool:
    if req.entry_price is None and req.stop_price is None and req.direction is None:
        return bool(req.structural_stop_valid)
...
def authorize_risk(req:RiskRequest, limits:RiskLimits, state:RiskState, *, min_rr:float=1.0,
    ...
    approved=not reasons; current=_utc(now)
