...
def orchestrate_paper(opportunity:Mapping[str,Any],auth:Mapping[str,Any],ledger:Sequence[Mapping[str,Any]]|None,*,watchdog_state:str="NORMAL",now=None)->dict[str,Any]:
    ...
    row={"paper_request_id":req["paper_request_id"],"opportunity_id":oid,"pair":req["pair"],"strategy_version":req["strategy_version"],"risk_auth_id":req["risk_auth_id"],"direction":req.get("direction"),"authorized_entry_price":req.get("authorized_entry_price"),"authorized_stop_price":req.get("authorized_stop_price"),"auth_expires_at":req.get("auth_expires_at"),"max_risk":req.get("max_risk"),"max_exposure":req.get("max_exposure"),"status":"WAIT_ENTRY","environment":"PAPER","real_orders_enabled":False}
