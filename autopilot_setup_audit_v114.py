...
def build_summary(audit: pd.DataFrame, performance: pd.DataFrame, *, now: pd.Timestamp | None = None) -> dict[str, Any]:
    ...
    explicit_setups=(
        ...
        if "setup_id" in d.columns else pd.Series(False,index=d.index)
    )
    explicit_setup_closed=(
        closed["setup_id"].fillna("").astype(str).str.strip().ne("")
        if isinstance(closed,pd.DataFrame) and "setup_id" in closed.columns else pd.Series(False,index=closed.index)
    )
    return {
