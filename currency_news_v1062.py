"""USD Macro Pro V10.6.2 — Snapshot Recovery & Fresh-Price Backfill.
...
ou histórico oficial. A V10.6.2 mantém a calibração global e acrescenta recuperação segura
de preço M15 fresco para snapshots do mesmo dia, sem criar linhas duplicadas.
"""
...
def _validation_columns_v1061() -> list[str]:
    return [
        ...
        "signal_frozen_at", "entry_origin", "backfilled_at",
...
def render_news_validation_panel_v1061(
    ...
    st.markdown("### 🧪 Calibração & Validação — V10.6.2")
    st.caption(
        "1 par + 1 dia = 1 snapshot. Se o preço estava ausente, a MESMA linha pode receber "
        "um preço M15 fresco depois, sem criar duplicidade. A validação continua em 1H, 4H e 24H."
    )
...
def render_currency_news_panel(
    ...
    st.subheader("🌍 Currency News Intelligence — V10.6.2")
    st.caption(
    ...
    with st.expander("🛡️ O que mudou na V10.6.2"):
        st.markdown(
