...
class FreshnessIntegrityV1103Tests(unittest.TestCase):
    def _scanner(self, now, m15_age=20, h1_age=60, h4_age=120, m15_bars=40, h1_bars=20):
        return {
            ...
            "tecnico": {
                ...
                "m15": {"status":"🟢 GATILHO"},
                "cache_v110": {
                    "m15": [{} for _ in range(m15_bars)],
                    "h1": [{} for _ in range(h1_bars)],
                },
            },
        }
    ...
    def test_fresh_h1_m15_allows_ict(self):
        ...
        dr=assess_pair_data_readiness(self._scanner(now), {"w1_bias":"ALTISTA"}, now=now)
    ...
    def test_stale_m15_masks_amd_fvg(self):
        now=pd.Timestamp('2026-09-15T02:00:00Z')
        dr=assess_pair_data_readiness(self._scanner(now,m15_age=113), {"w1_bias":"ALTISTA"}, now=now)
    ...
    def test_stale_m15_does_not_mask_h1_crt_ote_if_h1_is_current(self):
        now=pd.Timestamp('2026-09-15T02:00:00Z')
        dr=assess_pair_data_readiness(self._scanner(now,m15_age=113,h1_age=60), {"w1_bias":"ALTISTA"}, now=now)
        for key in ("crt","ote"):
    ...
    def test_stale_h1_masks_crt_ote(self):
        now=pd.Timestamp('2026-09-15T02:00:00Z')
        dr=assess_pair_data_readiness(self._scanner(now,h1_age=250), {"w1_bias":"ALTISTA"}, now=now)
        for key in ("crt","ote"):
            s,t,current=display_ict_component_status({"status":"🟢 ANTIGO","text":"old"},key,dr)
            self.assertFalse(current)
