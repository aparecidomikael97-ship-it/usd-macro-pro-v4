...
class ProductionBootContractTests(unittest.TestCase):
    ...
    def test_production_smoke_requires_current_home_when_not_at_login(self):
        ...
        self.assertIn('initial_private_home = (',workflow)
        self.assertIn('"Safety Core monitorado" in initial_body_text',workflow)
        self.assertIn("private_home_visible = initial_private_home",workflow)
        self.assertIn("produção carregou Streamlit, mas não abriu uma Home válida",workflow)
        self.assertIn("Home/Radar demorou",workflow)
        self.assertIn("measured_ui_ms is not None and measured_ui_ms > 30000",workflow)
        self.assertIn("measured_ui_ms_excluding_deploy_wait",workflow)
