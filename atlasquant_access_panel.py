...
from atlasquant_access_control import authenticate, load_users_config, has_permission
...
def evaluate_access(*, required:bool, users_count:int, session:dict[str,Any]|None)->dict[str,Any]:
    if not required:
...
def render_access_gate()->dict[str,Any]:
    ...
    decision=evaluate_access(required=required,users_count=len(users),session=session)
    if not required:
