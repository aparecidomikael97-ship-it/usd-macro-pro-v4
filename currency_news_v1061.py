"""USD Macro Pro V10.6.1 — News Calibration & Validation.
...
ou histórico oficial. A V10.6.1 calibra duplicidade global, separa cobertura de convicção
e registra/valida snapshots em 1H, 4H e 24H antes de qualquer integração ao Score Mestre.
"""
...
def _scanner_price_map_v1061(max_age_minutes: float = 60.0) -> dict[str, dict[str, Any]]:
    ...
    for pair, raw in results.items():
        ...
        stamp = _parse_state_time_v1061(raw.get("processado_em"))
...
def register_daily_news_snapshot_v1061(
    ...
    if not df.empty:
        existing = set(
            df["day_utc"].astype(str) + "|" + df["pair"].astype(str)
        )

    added = 0
    directional_with_price = 0
    new_rows = []
    ...
    for _, row in pair_df.iterrows():
        ...
        if side in ("BUY", "SELL") and entry_price is not None:
            status = "PENDENTE"
            directional_with_price += 1
        elif side == "NEUTRAL":
        ...
        new_rows.append({
            ...
            "price_source": "Scanner M15 <=60min" if entry_price is not None else "Sem preço técnico fresco",
    ...
    if new_rows:
        ...
        ok, save_err = _gh_write_csv_v1061(
            _NEWS_VALIDATION_PATH, df[cols],
            "V10.6.1: registrar snapshot diário de notícias"
        )
        if not ok:
            return False, save_err, df

    return True, (
        f"{added} par(es) registrados hoje; "
        f"{directional_with_price} com direção + preço fresco para validação."
    ), df
...
def render_news_validation_panel_v1061(
    ...
    st.markdown("### 🧪 Calibração & Validação — V10.6.1")
    st.caption(
        "Registra 1 fotografia por par/dia e mede a direção das notícias em 1H, 4H e 24H. "
        "Isso é o que vamos usar antes de permitir qualquer peso no Score Mestre."
    )
    ...
    show_cols = [
        ...
        "coverage", "conviction", "entry_price", "validation_status",
...
def render_currency_news_panel(
    ...
    st.subheader("🌍 Currency News Intelligence — V10.6.1")
    st.caption(
    ...
    with c_info:
        st.info(
            "V10.6.1: deduplicação também ENTRE moedas + peso de independência + "
    ...
    with st.expander("🛡️ O que mudou na calibração V10.6.1"):
        st.markdown(
