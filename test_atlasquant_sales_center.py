...
class AtlasQuantSalesCenterTests(unittest.TestCase):
    ...
    def test_commercial_readiness_distinguishes_internal_guide_from_external_features(self):
        ...
        self.assertTrue(status["academy_storyboards_ready"])
        ...
        self.assertTrue(status["data_provider_inventory_ready"])
        self.assertFalse(status["data_licenses_verified"])
        ...
        self.assertTrue(status["native_packaging_preparation_ready"])
    ...
    def test_onboarding_distinguishes_text_academy_from_pending_videos(self):
        ...
        self.assertEqual(by["Academy"]["Status"],"TEXTO + ROTEIROS + STORYBOARDS PRONTOS · VÍDEOS PENDENTES")
    ...
    def test_sales_checklist_surfaces_internal_security_controls(self):
        ...
        self.assertIn("Preparação de lançamento",src)
        self.assertIn("Finalização interna do AtlasQuant: COMPLETA",src)
        self.assertIn("Pendências externas para lançamento público",src)
